...
if( $active_user->id == $this_project->user_id ) {
	...
	$this_project->mailEveryone("[ACH] New user in project '" . $this_project->title . "'", "Hello,\r\n\r\n" . $this_user->name . " has joined the project '" . $this_project->title . "':\r\n " . $base_URL . "project/" . $this_project->id . "\r\n\r\n - The ACH Bot");
		$result = achquery("INSERT INTO users_in_projects (project_id, user_id) VALUES ('$project_id', '$user_id')");
		$result = achquery("INSERT INTO invitation_notices (user_id, by_user_id, project_Id, type, message) VALUES ('$user_id', '$active_user->id', '$project_id', 'approve', '');");
		...
		$result = achquery("INSERT INTO invitation_notices (user_id, by_user_id, project_Id, type, message) VALUES ('$user_id', '$active_user->id', '$project_id', 'deny', '');");
		sendMail($this_user->email, "[ACH] Project join request denied.", "Hello,\r\n\r\nYou have been denied access to project '" . $this_project->title . "'.\r\n\r\n - The ACH Bot");
	}
	achquery("DELETE FROM join_requests WHERE user_id='$user_id' AND project_id='$project_id'");
}
