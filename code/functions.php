...
function showClassification($classification_code) {
	if ($classification_code == "U") {
		return "Unclassified";
	}
	if ($classification_code == "C") {
		return "Classified";
	}
	if ($classification_code == "S") {
		return "Secret";
	}
	if ($classification_code == "TS") {
		return "Top Secret";
	}
	if ($classification_code == "Compartmented") {
...
function getStDev($numbers) {// Gets standard deviation. Takes an array called $numbers.
	...
	if ($n <= 1) {
		...
		$i = 0;
		while ($i < $n) {
			$x_minus_mean = $numbers[$i] - $mean;
			// x - Xbar
			$x2[$i] = $x_minus_mean * $x_minus_mean;
			// square above
			$i++;
		}
		$x2_total_sum = array_sum($x2);
		// sum up all x - Xbar square
		$x2_div_n = $x2_total_sum / ($n);
		// divide above by number of elements
		$stdDev = sqrt($x2_div_n);
		// get square root for standard deviation
...
function getActiveUsers($project_id) {
	...
	$result = mysql_fetch_array("SELECT user_id FROM users_active;");
	// WHERE last_visited >= $ten_minutes_ago;");
	print_r($result);
	while ($query_data = mysql_fetch_array($result)) {
