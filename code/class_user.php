...
class User extends FrameworkDatabase {
	...
	public function checkLoggedIn() { // Returns a boolean as to whether the current User object is the User that's logged in.		
		global $cookie_user_id, $cookie_user_password;
		$status = FALSE;
		if( strcmp($cookie_user_id, "") !== 0) {
			if ( strcmp($this->id, $cookie_user_id)===0 && strcmp($this->password, $cookie_user_password)===0 ) {
				$status = TRUE;}}
		return $status; 
	}
	...
	public function populateFromAttribute($value, $kind) { // Populates all of the varibles in this object from the DB.
		...
		$results = mysql_fast("SELECT * FROM users_active, user WHERE users_active.user_id = users.user_id AND user.$kind='$value' LIMIT 1");
		for( $i = 0; $i < count($results); $i++ ) {
			foreach ($results[$i] as $field => $value) {
				$this->$field = $value;
			}
	...
	public function changeUsername($new_username) { // Changes the user's nickname.
		...
		if( $this->checkUsernameAvailability($new_username) ) {
			// Then do the thing.
