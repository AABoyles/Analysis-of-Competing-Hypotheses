...
//if the matrix has no hypotheses, tell the user they need to add them...
				...
				$count = mysql_num_rows($result);
				 if ($count == 0) {
					if( $active_user->id == $active_project->user_id)	{	
						...
						<p><a href=\"" . $base_URL . "help/hypotheses.php\">Read more about choosing good hypotheses</a>.</p>"; 
						}
		...
		<select id="hypotheses_column_sort" onChange="setStyleByClass('th','hypothesis_colgroup_added','display','none'); setStyleByClass('th','hypothesis_colgroup_alpha','display','none'); setStyleByClass('th','hypothesis_colgroup_least_likely_personal','display','none'); setStyleByClass('th','hypothesis_colgroup_most_likely_personal','display','none'); setStyleByClass('th','hypothesis_colgroup_'+this.options[selectedIndex].value,'display',''); setStyleByClass('td','colgroup_added','display','none'); setStyleByClass('td','colgroup_alpha','display','none'); setStyleByClass('td','colgroup_least_likely_personal','display','none'); setStyleByClass('td','colgroup_most_likely_personal','display','none'); setStyleByClass('td','colgroup_'+this.options[selectedIndex].value,'display',''); sortColsBy = this.options[selectedIndex].value;">
			<option value="added">Date Added</option>
			<option value="most_likely_personal">Most Likely</option>
			<option value="least_likely_personal">Least Likely</option>
		...
		<select id="select_sort_1">
			...
			<option value="cred_weight" <?php if( $sort_field_1 == "cred_weight" ) { echo("selected "); }?>>Cred Weight</option>
		</select>
		...
		<select id="select_sort_2">
			...
			<option value="cred_weight" <?php if( $sort_field_2 == "cred_weight" ) { echo("selected "); }?>>Cred Weight</option>
		</select>
		...
		<?php } ?> <!--close out "if matrix exists, show matrix" --> 

<?php } ?> <!--close out "if user is member" -->


<?php 
//if this project is public and the user isn't a member, tell them what they can do...
		if( ($active_project->public =="y" &! in_array($active_user->id, $active_project->users)) || in_array($active_user->id, $active_project->users_view_only) ) { ?>
		<p>You must join this project in order to have a Personal Matrix.<br />
		<?php 
...
<!--now show all the other tools: compare matrices, view other... -->
